...
| 能力领域 | OmniFic 可以做什么 |
| --- | --- |
| **长篇写作** | 管理项目、封面、卷、章节与多级笔记；自动保存、编辑器内查找替换、章节排序与 Markdown 导出 |
| **人物与设定** | 为项目维护角色档案、头像和世界书条目，检索资料并导出通用 JSON 文件 |
...
- 在统一三栏工作区中切换章节与笔记，并按需收起、恢复资料栏、编辑区和 Agent 栏
...
- 将当前章节或笔记的实时草稿直接导出为 UTF-8 Markdown 文件
...
- 将单个世界书条目导出为 SillyTavern 兼容 JSON，将角色档案导出为 `chara_card_v2` 角色卡 JSON
...
- 为新项目首次正文写入检查世界书、主要角色与两级大纲，由 Auditor 审查并要求当前回合明确授权
- 在会话记录中保留模型切换提示，并以连续问答方式处理单选、多选和自定义澄清问题
