...
- 完善首次正文写作准入、会话生命周期与写作准备状态，避免 Agent 在关键信息不足时直接开写
- 在会话中展示模型切换、写作准入和执行反馈，并重构澄清问题的回答、跳过与恢复流程
- 修复子智能体数量较多时列表超出侧栏且无法查看底部内容的问题
- 为章节、笔记、世界书条目和角色卡增加文件导出功能
- 统一项目、世界书和角色工作区的三栏收起、恢复、栏高及按钮样式
- 精简项目编辑区，移除无实际价值的浏览器标签页式栏位，并将编辑区收起入口并入工具栏
